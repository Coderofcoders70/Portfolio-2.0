...
import Card from "./Card"; // Import your new component
import { FiGithub, FiTwitter, FiMapPin, FiMail } from "react-icons/fi"; // Example icons
import { SiReact, SiNodedotjs, SiMongodb, SiTailwindcss, SiTypescript, SiGoogle } from "react-icons/si";

function App() {

  const [aboutMode, setAboutMode] = React.useState<'work' | 'life'>('work');
    ...
    <>
      <div className="min-h-screen bg-slate-950 text-slate-200 p-4 md:p-8 font-sans selection:bg-cyan-500/30">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-4 auto-rows-[minmax(180px,auto)]">
          ...
          {/* 1. HERO Card (2x2) */}
          <Card className="md:col-span-2 md:row-span-2 flex flex-col relative overflow-hidden group">

            {/* 1. Background Glow Spot (Adds depth behind the text) */}
            <div className="absolute -right-20 -top-20 w-64 h-64 bg-cyan-500/20 rounded-full blur-3xl pointer-events-none" />

            {/* 2. Top Section: Status & Image */}
            <div className="flex justify-between items-start z-10">
              <div className="flex flex-col gap-2">
                {/* "Available" Badge with a pulsing dot */}
              ...
              <div className="relative w-20 h-20 md:w-30 md:h-30 rounded-full border-2 border-white/10 bg-white/5 overflow-hidden">
                {/* REPLACE THIS URL with your Memoji or Photo */}
                <img
          ...
          <Card className="md:col-span-1 md:row-span-2 flex flex-col">
            ...
            <div className="flex justify-between items-center mb-6">
              ...
              <div className="flex bg-slate-800 p-1 rounded-full border border-white/5">
                <button
                  onClick={() => setAboutMode('work')}
                  className={`px-3 py-1 rounded-full text-xs font-bold transition-all ${aboutMode === 'work' ? 'bg-slate-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
                >
                  Work
                </button>
                <button
                  onClick={() => setAboutMode('life')}
                  className={`px-3 py-1 rounded-full text-xs font-bold transition-all ${aboutMode === 'life' ? 'bg-cyan-600 text-white shadow-lg' : 'text-slate-500 hover:text-slate-300'}`}
                >
                  Life
                </button>
              </div>
              ...
              {aboutMode === 'work' ? (
                <div key="work" className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <p className="text-slate-400 leading-relaxed mb-4">
                    I am a <span className="text-white font-bold">Full Stack Engineer</span> focused on scalable backends and intuitive frontend architectures.
                  </p>
                ...
                <div key="life" className="animate-in fade-in slide-in-from-bottom-2 duration-300">
                  <p className="text-slate-400 leading-relaxed mb-4">
                    When I'm not coding, I'm usually exploring <span className="text-orange-400">new tech trends</span> or refining my UI/UX taste.
                  </p>
                  <p className="text-slate-400 leading-relaxed">
                    I believe in <span className="text-white italic">"Shipping fast and breaking things"</span> (but fixing them faster).
                  </p>
                  <ul className="mt-4 space-y-2 text-sm text-slate-500">
                    <li>🎧 Lo-fi Music Playlists</li>
                    <li>☕ Black Coffee</li>
          ...
          {/* 3. EDUCATION (1x1) */}
          <Card className="md:col-span-1 md:row-span-1 flex flex-col justify-center items-center text-center group">
          ...
          {/* 4. LOCATION (1x1) */}
          <Card className="md:col-span-1 md:row-span-1 flex flex-col justify-center items-center">
          ...
          <Card className="md:col-span-2 md:row-span-2 flex flex-col justify-between group">
            ...
            <div className="flex justify-between items-start mb-4 relative z-10">
              <div>
                <h2 className="text-2xl font-bold text-slate-100">Smart Bot</h2>
                <p className="text-sm text-slate-500 mt-1">MERN Stack • Gemini API • Socket.io</p>
              </div>

              {/* The "Live" Badge */}
              <div className="flex items-center gap-2 px-2 py-1 bg-green-500/10 border border-green-500/20 rounded-md">
                <div className="w-1.5 h-1.5 rounded-full bg-green-500 animate-pulse" />
                <span className="text-xs font-bold text-green-400">V1.0 Live</span>
              </div>
            </div>

            {/* 2. The Visual Showcase (Browser Window Look) */}
            {/* We push this to the bottom so it gets 'cut off' intentionally, looking artistic */}
            <div className="relative w-full h-64 md:h-full bg-slate-900 rounded-t-xl border-t border-l border-r border-white/10 overflow-hidden shadow-2xl mt-4 group-hover:translate-y-[-5px] transition-transform duration-500">
              ...
              <div className="h-8 bg-slate-800 border-b border-white/5 flex items-center px-3 gap-1.5">
                ...
                <div className="ml-4 w-full max-w-[150px] h-4 bg-slate-700/50 rounded-full" />
              </div>

              {/* The Actual Project Image/Video Area */}
              <div className="relative w-full h-full bg-slate-950 flex flex-col p-4">

                {/* Mock Chat UI (Since you have a dashboard) */}
                <div className="flex flex-col gap-3">
                  {/* User Message */}
                  <div className="self-end bg-cyan-600/20 text-cyan-200 px-4 py-2 rounded-2xl rounded-tr-sm text-xs max-w-[80%] border border-cyan-500/20">
                    Analyze user retention rates for Q3.
                  </div>

                  {/* AI Response (Gemini) */}
                  <div className="self-start bg-slate-800 text-slate-300 px-4 py-2 rounded-2xl rounded-tl-sm text-xs max-w-[90%] border border-white/5">
                    <span className="text-purple-400 font-bold text-[10px] block mb-1">GEMINI AI</span>
                    Based on the data, retention is up by 15%. Here is the breakdown...
                  </div>

                  {/* A Chart Placeholder (Visuals matter!) */}
                  <div className="self-start mt-2 w-full h-24 bg-gradient-to-r from-purple-900/20 to-blue-900/20 rounded-xl border border-white/5 relative overflow-hidden">
                    {/* Add a fake graph line using SVG if you want, or just a screenshot later */}
                    <div className="absolute bottom-0 left-0 right-0 h-10 bg-cyan-500/10 skew-y-3" />
                  </div>
                </div>
          ...
          {/* 7. CONTACT (1x1) */}
          {/* <Card className="md:col-span-1 md:row-span-1 bg-gradient-to-br from-purple-600/20 to-blue-600/20">
          ...
          {/* 7. CONTACT CARD (1x1) - The "Let's Work" Button */}
          {/* This replaces the old simple 'Email Me' card with something bolder */}
          <Card className="md:col-span-1 md:row-span-1 bg-gradient-to-br from-purple-600 to-indigo-600 border-none relative overflow-hidden group">
            <a href="/lakshayas918@gmail.com" className="h-full w-full flex flex-col justify-between p-1">
          ...
          {/* 8. LEARNING CARD (1x1) - The "Progress Bar" */}
          {/* This is the code you liked, kept exactly the same */}
          <Card className="md:col-span-1 md:row-span-1 flex flex-col justify-center">
            <p className="text-xs text-slate-500 mb-2 font-bold uppercase tracking-wider">Currently Learning</p>

            <div className="flex justify-between items-end mb-2">
              <h3 className="font-bold text-xl text-slate-200">Next.js 15</h3>
              <span className="text-xs text-cyan-400 font-bold">75%</span>
            </div>

            {/* The Progress Bar */}
            <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden border border-white/5">
              <div className="bg-gradient-to-r from-cyan-500 to-blue-500 h-full w-[75%] animate-pulse" />
            </div>
          </Card>

          {/* 8. LEARNING (1x1) */}
          {/* <Card className="md:col-span-1 md:row-span-1">
            <p className="text-xs text-slate-500 mb-2">Learning</p>
            <h3 className="font-bold text-lg mb-2">Next.js 15</h3>
            <div className="w-full bg-slate-800 h-2 rounded-full overflow-hidden">
              <div className="bg-gradient-to-r from-cyan-500 to-blue-500 h-full w-[75%]" />
            </div>
          </Card> */}
